...
router.get(
  ...
  handlerHOC(async (req) => {
    ...
    return {code: 200, body: allAssets}
  }) 
);
router.get(
  "/pending",
  handlerHOC(async (req) => {
    ...
    return {code: 200, body: pendingAssets}
  }) 
);
router.get(
  "/suspended",
  handlerHOC(async (req) => {
    const suspendedAssets = await findAssetsByPublishingStatus(Status.Suspended)
    return {code: 200, body: suspendedAssets}
  }) 
);
router.put(
  "/approve/:id",
  handlerHOC(async (req) => {
    ...
    return {code: 201, body: `Listing ${listingId} is now Active`}
  }) 
);
...
router.put(
  ...
  handlerHOC(async (req) => {
    ...
    return {code: 201, body: `Listing ${listingId} is Suspended`}
  }) 
);
