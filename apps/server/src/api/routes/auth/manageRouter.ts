...
import passResetRequestModel from "../../../services/mongo/auth/passResetRequestModel";
import PassResetRequestModel from "../../../services/mongo/auth/passResetRequestModel";
import multer from "multer";
import { PutObjectCommand } from "@aws-sdk/client-s3";
import { bucketName, s3Client } from "../../../services/s3";
...
router.put(
  ...
  async (req: TODO, res, next) => {
    try {
      ...
      const userId = String(req.user._id); // Assuming _id is available and toString() is valid
...
router.delete("/profile-picture", async (req: TODO, res, next) => {
  try {
    if (!req.user) return res.status(401).send("Please Login");
    req.user.profilePictureUrlKey = undefined;
    await req.user.save();
